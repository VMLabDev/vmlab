...
TempleOS reads no ISO 9660 and has no network, so the bootstrap ISO cannot carry
...
Roughly twelve thousand keystrokes, about eight minutes at 40 ms, once per
template build. The machine must use the QMP input transport, the default: over
VNC, TempleOS sees each shift a keystroke late and shifted characters land on
the wrong key.
