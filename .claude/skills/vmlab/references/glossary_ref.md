...
| vmlab-agent | vmlab's first-party in-guest agent on the `vmlab.agent.0` virtio-serial port (no guest network). Installed by the guest's own unattended-install hook from the auto-attached VMLAB bootstrap ISO at build time (meta `agent_version`); powers readiness, `vmlab exec`/`shell`/`cp`/`tail`/`eventlog`/`osinfo`, wscript `terminal()`/`stats()`, and graceful shutdown/reboot. Present in both full VMs and container micro-VMs. | agent channel |
