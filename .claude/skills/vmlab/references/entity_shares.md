...
path requires a NIC on a segment, and so does the default `transport = "auto"`
(it can still fall back to SMB at VM start) — a validation error otherwise.
Only an explicit `transport = "virtiofs"` shares into a machine with no NICs at
all. On Windows the agent mounts as SYSTEM (visible to
