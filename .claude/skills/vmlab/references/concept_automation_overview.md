...
(virtio-serial, no guest network needed). There is no fallback: a guest
without the agent has no exec, no file copy and no readiness — which is why
templates built before the agent must be rebuilt. Vision-based automation
(screen matching, OCR) needs no agent at all — it reads the display, which is
what makes even vintage guests scriptable.
