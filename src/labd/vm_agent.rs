...
// The file vocabulary itself, re-exported so callers of this client never
// have to reach past it to name an operation (§19.5).
pub use vmlab_agent_proto::fileops::{Attrs, DirEntry, ErrorCode, LinkKind, Op, OpenFlags, Reply};
...
pub enum SessionEvent {
    ...
    /// Exec stderr.
    Stderr(Vec<u8>),
    /// No more guest→host bytes, and the channel is still open: an exec's
    /// output pipes drained.
...
/// How long an `open_*` waits for the agent's `opened`.
...
impl AgentHandle {
    ...
    async fn open(&self, build: impl FnOnce(u32) -> HostMsg) -> Result<AgentSession> {
        ...
        match tokio::time::timeout(OPEN_TIMEOUT, opened_rx).await {
            ...
            Ok(Err(_)) | Err(_) => {
                ...
                Err(anyhow!("agent did not open the channel in time"))
            }
        }
    }
    ...
    /// offset-addressed and pipelined. Every transfer below runs over one, and
    /// so does the workspace syncer, so there is only ever one file
    /// vocabulary.
    ///
...
impl FileOps {
    ...
    ///
    /// The transfers above are the convenient callers; this is for one that
    /// needs the vocabulary itself.
    ///
...
/// `push` branches on it (a missing parent directory is recoverable).
#[derive(Debug)]
...
/// A refusal carries the agent's message.
