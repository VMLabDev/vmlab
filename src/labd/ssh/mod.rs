...
//! channels onto agent channels: `session` onto a terminal or an exec,
//! `subsystem sftp` onto a `fileops` session ([`sftp`]), and `direct-tcpip`
//! onto a tunnel the *guest* dials. The transport is the agent channel, so a
//! machine on no segment at all is attachable — including for `ssh -D`, whose
//! destinations are reached from inside the guest — and a VM and a container
//! micro-VM are attached to the same way.
//!
